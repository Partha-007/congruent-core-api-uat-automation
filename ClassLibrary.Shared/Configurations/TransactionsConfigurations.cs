...
namespace ClassLibrary.Shared.Configurations
{
    public class TransactionsConfigurations : TestBase
    {
        ...
        public static string? _url, fundSwitchTransactionId;
        ...
        public async Task FundSwitchConfiguration(HttpClient httpClient, string ApplicableLevel, string InvestmentFrom, string InvestmentTo)
        {
            var program = new RefitSandBox.Program();
            var investmentFrom = await program.GetInvestmentIdAndCusip(InvestmentFrom);
            var investmentTo = await program.GetInvestmentIdAndCusip(InvestmentTo);

            var fundSwitchModel = new FundSwitchDetails()
            {
                Id = 0,
                TransactionReferenceId = null,
                Name = "Test Fund Switch",
                Description = "Test Fund Switch Description",
                EffectiveDate = DateTimeOffset.Now.AddDays(2),
                BlackOutPeriodStart = DateTimeOffset.Now.AddDays(1),
                BlackOutPeriodEnd = DateTimeOffset.Now.AddDays(3),
                SameAllocationToAllInvestments = true,
                ApplicableLevel = 2,
                FundSwitchStatus = 1,
                InvestmentsFrom = new List<FundSwitchInvestmentFromDetails>
                {
                    new FundSwitchInvestmentFromDetails
                    {
                        InvestmentId = int.Parse(investmentFrom.investmentId),
                        InvestmentName = InvestmentFrom,
                        Cusip = investmentFrom.CUSIP,
                        InvestmentsTo = new List<FundSwitchInvestmentToDetails>
                        {
                            new FundSwitchInvestmentToDetails
                            {
                                InvestmentId = int.Parse(investmentTo.investmentId),
                                InvestmentName = InvestmentTo,
                                Cusip = investmentTo.CUSIP,
                                AllocationPercentage = 100
                            }
                        }
                    }
                },
                Plans = new List<FundSwitchPlanMapping>
                {
                    new FundSwitchPlanMapping
                    {
                        PlanId = int.Parse(Hooks.planId!),
                        Id = 0
                    }
                },
                PlanGroups = null
            ...
            var fundSwitchInterface = RestService.For<IFundSwitch>(httpClient);
            var fundSwitchResponse = await fundSwitchInterface.SaveFundSwitchDetailsAsync(fundSwitchModel);

            if (!fundSwitchResponse.IsSuccessfull)
                throw new Exception("Fund switch save failed");

            fundSwitchTransactionId = fundSwitchResponse.FundSwitchDetails.TransactionReferenceId;
        }

        public async Task UpdateFundSwitchStatus()
        {
            var httpClient = new HttpClient()
            {
                BaseAddress = new Uri(_url!)
            ...
            httpClient.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", Hooks.bearer!);
            var fundSwitchInterface = RestService.For<IFundSwitch>(httpClient);
            var updateStatusResponse = await fundSwitchInterface.UpdateFundSwitchStatusAsync(2, fundSwitchTransactionId!);
            if (!updateStatusResponse)
                throw new Exception("Fund switch status update failed");
        }
